...
- GitHub Actions (`.github/workflows/atualizar.yml` e `atualizar_free_agents.yml`): **schedules removidos em 2026-08-27** — `workflow_dispatch:` mantido como plano B manual. A coleta agora é feita exclusivamente pelo cron do servidor. Histórico: o `atualizar_free_agents.yml` falhou desde 2026-08-26 18:12 UTC (bug em `gerar_comps_finais.py`: `TypeError` ao formatar `salario_medio=None` no bloco de display da amostra — corrigido em 2026-08-27). A migração pro cron do servidor estava pela metade; concluída em 2026-08-28 (primeiro commit com formato `Atualização free-agents … UTC` com hífen confirmando o caminho novo funciona).
...
6. ~~Desativar os `schedule:` do GitHub Actions~~ — **RESOLVIDO em 2026-08-27**: schedules removidos de ambos os workflows após confirmar o cron do servidor funcionando (primeiro commit `Atualização free-agents` com hífen em 2026-08-28 03:55 UTC)
